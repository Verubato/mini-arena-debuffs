...
local function RefreshHeaderChildSizes(header)
	local iconSize = tonumber(db.Icons.Size) or 32
...
---@param unit string
---@param header table
local function UpdateHeader(header, unit)
	local iconSize = tonumber(db.Icons.Size) or 32
	...
	header:SetAttribute("xOffset", iconSize + (tonumber(db.Icons.Padding.X) or 0))
	-- no yoffset padding until we wrap
	header:SetAttribute("yOffset", 0)
	header:SetAttribute("wrapAfter", tonumber(db.IconsPerRow) or 5)
	...
	header:SetAttribute("wrapYOffset", -iconSize - (tonumber(db.Icons.Padding.Y) or 0))
	header:SetAttribute("sortMethod", tostring(db.SortMethod) or "INDEX")
	header:SetAttribute("sortDirection", db.SortDirection or "+")
...
local function CreateSecureHeader()
	...
	header:SetAttribute("filter", "HARMFUL|PLAYER")
...
---@param unit string
---@param header table
function M:UpdateHeader(header, unit)
	UpdateHeader(header, unit)
end
